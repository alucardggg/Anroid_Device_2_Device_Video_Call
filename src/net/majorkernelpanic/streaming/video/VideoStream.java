...
import net.majorkernelpanic.p2p.main.AvcEncoder;
...
public abstract class VideoStream extends MediaStream {
	...
	protected AvcEncoder mAvcCodec;
	protected byte[] mH264 = new byte[320 * 240 * 3 / 2];
	...
	protected void encodeWithMediaRecorder() throws IOException,
		...
		try {
			...
			mMediaRecorder.setPreviewDisplay(mSurfaceView.getHolder()
					.getSurface());
			mMediaRecorder.setVideoSize(mRequestedQuality.resX,
					mRequestedQuality.resY);
			mMediaRecorder.setVideoFrameRate(mRequestedQuality.framerate);

			// The bandwidth actually consumed is often above what was requested
			mMediaRecorder
					.setVideoEncodingBitRate((int) (mRequestedQuality.bitrate * 0.8));

			// We write the ouput of the camera in a local socket instead of a
			// file !
	 ...
	 */
	@SuppressLint("NewApi")
	protected void encodeWithMediaCodecMethod1() throws RuntimeException,
			IOException {
		...
		// mAvcCodec = new AvcEncoder(mQuality.resX, mQuality.resY,
		// mQuality.framerate, mQuality.bitrate);
...
//				Log.e("preview time stamp = ",  System.currentTimeMillis()+"");
				// int ret = mAvcCodec.offerEncoder(data, mH264);
				// try {
				// VideoStream_Test.writeToFile(mH264, ret,
				// VideoStream_Test.getFile());
				// } catch (IOException e) {
				// // TODO Auto-generated catch block
				// e.printStackTrace();
				// }
				oldnow = now;
				now = System.nanoTime() / 1000;// - 1000000;
				if (i++ > 3) {
					i = 0;
					// Log.d(TAG,"Measured: "+1000000L/(now-oldnow)+" fps.");
				}
				try {
					// int bufferIndex = mMediaCodec.dequeueInputBuffer(200000);
					int bufferIndex = mMediaCodec.dequeueInputBuffer(-1);
					if (bufferIndex >= 0) {
						inputBuffers[bufferIndex].clear();
						if (data == null)
							Log.e(TAG,
									"Symptom of the \"Callback buffer was to small\" problem...");
						else
		...
		/* ************************* only for test ***********************/
//		final MediaCodecInputStream ms = new MediaCodecInputStream(mMediaCodec);
//
//		final int size = mQuality.resX * mQuality.resY * 3 / 2;
//		final byte[] h264 = new byte[size];
//
...
//				while (true) {
//					try {
//						int ret = ms.read(h264, 0, size);
//						VideoStream_Test.writeToFile(h264, ret,
//								VideoStream_Test.getFile());
//					} catch (IOException e) {
//						// TODO Auto-generated catch block
//						e.printStackTrace();
...
//		}).start();
	...
	/**
	 * Video encoding is done by a MediaCodec. But here we will use the
	 * buffer-to-surface method
	 */
	@SuppressLint({ "InlinedApi", "NewApi" })
	protected void encodeWithMediaCodecMethod2() throws RuntimeException,
			IOException {
	...
	private void openCamera() throws RuntimeException {
		...
		mCameraThread.start();
		// add by alucard, worker Thread wait for another thread open camera
		// then step forward
	...
	protected synchronized void createCamera() throws RuntimeException {
		...
		if (mCamera == null) {
			...
			try {
				...
				// parameters.setPreviewSize(mRequestedQuality.resX,
				// mRequestedQuality.resY);
				...
				try {
					if (mMode == MODE_MEDIACODEC_API_2) {
						...
						mCamera.setPreviewTexture(mSurfaceView
								.getSurfaceTexture());
					} else {
