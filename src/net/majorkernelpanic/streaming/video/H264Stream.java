 ...
 * stream. Call {@link #stop()} to stop the stream. Modified by Alucard Wang
 */
public class H264Stream extends VideoStream {
	...
	private MP4Config testH264() throws IllegalStateException, IOException {
		if (mMode != MODE_MEDIARECORDER_API) {
			...
			throw new IOException("media recorder is not supported!!");
		}
	}
	...
	private MP4Config testMediaCodecAPI() throws RuntimeException, IOException {
		...
		// Using the MediaCodec API with the buffer method for high resolutions
		// is too slow
	...
	private MP4Config testMediaRecorderAPI() throws RuntimeException,
			IOException {
		String key = PREF_PREFIX + "h264-mr-" + mRequestedQuality.framerate
		...
		final String TESTFILE = Environment.getExternalStorageDirectory()
				.getPath() + "/spydroid-test.mp4";
		...
		// Save flash state & set it to false so that led remains off while
		// testing h264
		...
		try {
					...
					.setOnInfoListener(new MediaRecorder.OnInfoListener() {
						public void onInfo(MediaRecorder mr, int what, int extra) {
							...
							if (what == MediaRecorder.MEDIA_RECORDER_INFO_MAX_DURATION_REACHED) {
								Log.d(TAG,
										"MediaRecorder: MAX_DURATION_REACHED");
							} else if (what == MediaRecorder.MEDIA_RECORDER_INFO_MAX_FILESIZE_REACHED) {
								Log.d(TAG,
										"MediaRecorder: MAX_FILESIZE_REACHED");
			...
			if (mLock.tryAcquire(6, TimeUnit.SECONDS)) {
				...
				Log.d(TAG,
						"MediaRecorder callback was not called after 6 seconds... :(");
			}
		} catch (IOException e) {
			...
			if (previewStarted) {
				// If the preview was started before the test, we try to restart
				// it.
				try {
					startPreview();
				} catch (Exception e) {
		...
		if (mSettings != null) {
			...
			editor.putString(key,
					config.getProfileLevel() + "," + config.getB64SPS() + ","
							+ config.getB64PPS());
			editor.commit();
		}
