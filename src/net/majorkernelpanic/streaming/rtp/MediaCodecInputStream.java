...
public class MediaCodecInputStream extends InputStream {
	...
	public int read(byte[] buffer, int offset, int length) throws IOException {
		...
		try {
			if (mBuffer == null) {
				while (!Thread.interrupted() && !mClosed) {
					mIndex = mMediaCodec.dequeueOutputBuffer(mBufferInfo,
							500000);
					if (mIndex >= 0) {
			...
			min = length < mBufferInfo.size - mBuffer.position() ? length
		...
		// add by alucard
//		if (buffer[4] == 0x65 && SPS_PPS == null) // 关键帧
//		{
//			SPS_PPS = new byte[] { 0, 0, 0, 1, 103, 66, 0, 41, -115,
//					-115, 96, 80, 30, -60, 0, 0, 0, 1, 104, -54, 67, -56 };
//
//		} else if(buffer[4] == 0x65 && SPS_PPS != null) {
//			
//			byte[] yuv420 = new byte[mBufferInfo.size];
//
//			System.arraycopy(buffer, 0, yuv420, 0, min);
//			System.arraycopy(SPS_PPS, 0, buffer, 0, SPS_PPS.length);
//			System.arraycopy(yuv420, 0, buffer, SPS_PPS.length, min);
//			return min + SPS_PPS.length;
//		}
